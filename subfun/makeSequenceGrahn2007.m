function seq = makeSequenceGrahn2007(cfg,seqi,varargin)
...
%         nSoundsPerPattern = [seq(patIdx).nSounds]; 
        ...
        % do the nonmetric (random) scambling 
        if isfield(cfg.pattern,'doNonMetric') && strcmp(currCategLabel,'complex')
            ...
            for IOIClass=IOIClasses
                ...
                % pretty much gridIOI)
                if IOIClass==1
                    if cfg.pattern.eventDur > currGridIOI*(1-ioiScrambleRatio)
                        continue
                    end
                end
                ...
                % if the number of events with this IOI value cannot be exactly 
                % split into 3 groups, we need to skip some events during the jitterring
                nJitterGroups = 2; 
                
                % find how many events we need to keep intact
                nToSkip = mod(length(idxIOIClass), nJitterGroups); 
                
                nToJitter = floor(length(idxIOIClass)/nJitterGroups);
                if nToJitter<1
                    warning(sprintf('In step:%d, segm:%d \nToo little instances for IOI=%d. Cannot split the IOIs into %d equal groups...\n\nSkipping this IOI category here...', ...
                        stepi, segmi, IOIClass, nJitterGroups)); 
                    continue
                end
                
                idxToSkip = randsample(idxIOIClass, nToSkip); 
                idxIOIClass(ismember(idxIOIClass,idxToSkip)) = []; 
                
%                 idxToKeep = randsample(idxIOIClass, nToJitter); 
%                 idxIOIClass(ismember(idxIOIClass,idxToKeep)) = []; 

                idxToLenghten = randsample(idxIOIClass, nToJitter); 
                idxIOIClass(ismember(idxIOIClass,idxToLenghten)) = []; 

                % if there's only 1 element left, we have to be careful with
                % randsample! 
                if length(idxIOIClass)==1
                    idxToShorten = idxIOIClass; 
                else
                    idxToShorten = randsample(idxIOIClass, nToJitter); 
                end
                idxIOIClass(ismember(idxIOIClass,idxToShorten)) = []; 
                
                if ~isempty(idxIOIClass)
                    error('We didnt use all IOIs during jittering!!! There is a problem somewhere');
                end
